module nsga2_module
	...
	public :: crossover, mutation, mutation_ad, mutation_ad_y, evaluate_peak_rsd, evaluate_peak_rsd_ad, &
	& pareto_y, evaluate_peak_rsd_y, check_k, crowd_dist, delete_z_i
	...
	integer ::	num_peak = 0	! peak numbers of the spectrums
	integer ::	num_freq = 0	! numbers of the kinds of chemical shifts
	real, allocatable ::	ch_shifts(:,:)	! chemical shifts
	real, allocatable :: e_width(:,:)	! resolution (half width)
	integer, allocatable ::	degeneracy(:)	! degeneracy number
	character*20, allocatable	::	poss_rsd(:)	! possible residue corresponding to each peak
	character*100, allocatable	::	poss_rsd_str(:)	! possible residue including secondary structure
	integer, allocatable ::	num_used(:)		! the used times of each peak

	integer, allocatable ::	peak_seq(:,:)	! the possible peaks for each residue
	integer, allocatable ::	num_poss_peak(:)! number of possible peaks for each residue
	integer, allocatable ::	prsd_peak(:,:)	! the possible residues for each peak
	integer, allocatable ::	num_poss_rsd(:)	! number of possible residues for each peak
	...
	!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	subroutine crossover(parents2, info_spectrum, connection_table, N_seq, n_table, cnn_row, children2, err_sign)
	! parent2 contains two individuals in the generation group
	! choose a random residue p, exchange the rd_pk(p, ind_table) in parent1 and  rsd_pk(p, ind_table) in parent2
	! to make children2 (two children)
	! And also calculate the evaluation values of the two children
	!
	implicit none
	!
	...
	integer	ngood_new, ngood_old, nbad_new, nbad_old, nedge_new, nedge_old, nused_new, nused_old
	!
	integer	stop_sign, i
	!
	...
	do i = 1, num_pos
		...
		!
		do k = 1, 2
			if (new_asg_2(k) .ne. 0 .and. new_asg_2(k) .ne. rsd_peak_2(ind_rsd, idx_table, k)) then
				p1 = 0
				do p2 = 1, N_seq
					if (rsd_peak_2(p2, idx_table, k) .eq. new_asg_2(k)) p1 = p1+1
				end do
		...
		if (new_asg_2(1) .eq. rsd_peak_2(ind_rsd, idx_table, 1) .or. new_asg_2(2) .eq. rsd_peak_2(ind_rsd, idx_table, 2)) then
			stop_sign = 0
		else
			stop_sign = 1
			exit
	...
	do k2 = 1, 2
		...
		call evaluate_peak_rsd (children2(k2)%rsd_pk, info_spectrum, connection_table, n_table, N_seq, cnn_row, &
		& idx_table, new_idx, new_asg, ngood_new, nbad_new, nedge_new)
		call evaluate_peak_rsd (children2(k2)%rsd_pk, info_spectrum, connection_table, n_table, N_seq, cnn_row, &
		& idx_table, new_idx, old_asg, ngood_old, nbad_old, nedge_old)
		!
	...
	!					mutation subroutine
	!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	subroutine mutation(parent1, info_spectrum, connection_table, N_seq, n_table, cnn_row, p_null, child1)
	! rsd_peak_k is the kth individual of the generation group
	! choose a random residue p, change the rsd_peak_k(p, ind_table) value to make a new assignment
	! new_asg & new_idx will be the output result
	!
	implicit none
	!
	integer, intent(in) ::	N_seq, n_table, cnn_row
	...
	do while (stop_sign .eq. 0)
		...
		new_idx = ind_rsd
		if (new_asg .ne. 0 .and. new_asg .ne. rsd_peak_k(ind_rsd, idx_table)) then
			p1 = 0
			do p2 = 1, N_seq
	...
	call evaluate_peak_rsd(rsd_peak_k, info_spectrum, connection_table, n_table, N_seq, cnn_row,&
	& idx_table, new_idx, new_asg, ngood_new, nbad_new, nedge_new)
	call evaluate_peak_rsd(rsd_peak_k, info_spectrum, connection_table, n_table, N_seq, cnn_row,&
	& idx_table, new_idx, old_asg, ngood_old, nbad_old, nedge_old)
	!
	...
	!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	!					additional mutation subroutine
	!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	subroutine mutation_ad_1(parent1, info_spectrum, connection_table, N_seq, n_table, cnn_row, child1, err_sign)
	! rsd_peak_k is the kth individual of the generation group
	! choose a random residue p1, and another random residue p2, exchange rsd_peak_k(p1, ind_table) and rsd_peak_k(p2, ind_table)
	! new_asg & new_idx will be the output result
	!
	implicit none
	!
	...
	integer, intent(out) ::	err_sign
	!
!	integer	::	new_asg_2n(n_table, 2), new_idx_2n(n_table, 2), old_asg_2n(n_table, 2)
	...
	do k = 1, num_pos
		...
		if (new_asg_2(1) .ne. 0) then
			if (all(info_spectrum(idx_table)%prsd_peak(:, new_asg_2(1)) .ne. new_idx_2(1))) 	then
				new_asg_2(1) = 0
			end if
		end if
		exit
	...
	do k2 = 1, 2
		...
		call evaluate_peak_rsd(rsd_peak_k, info_spectrum, connection_table, n_table, N_seq, cnn_row,&
		& idx_table, new_idx_2(k2), new_asg, ngood_new, nbad_new, nedge_new)
		call evaluate_peak_rsd(rsd_peak_k, info_spectrum, connection_table, n_table, N_seq, cnn_row,&
		& idx_table, new_idx_2(k2), old_asg, ngood_old, nbad_old, nedge_old)
		!
		if (old_asg .ne. 0) then
			nused_old = 1
		else
	...
	!					additional mutation subroutine
	!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	subroutine mutation_ad_2(parent1, info_spectrum, connection_table, N_seq, n_table, cnn_row, child1, err_sign)
	! rsd_peak_k is the kth individual of the generation group
	! choose a random residue p1, and another random residue p2, exchange rsd_peak_k(p1, ind_table) and rsd_peak_k(p2, ind_table)
	! new_asg & new_idx will be the output result
	!
	...
	do k = 1, num_pos
		...
		if (new_asg_2n(idx_table, 1) .ne. 0) then
			if (all(info_spectrum(idx_table)%prsd_peak(:, new_asg_2n(idx_table, 1)) .ne. new_idx_2(1))) 	then
				new_asg_2n(idx_table, 1) = 0
			end if
		end if
		exit
	end do
	if (all((new_asg_2n(idx_table,:)-old_asg_2n(idx_table,:)) .eq. 0) .or. new_idx_2(2) .eq. 0) then
		err_sign = 1
		return
	else
		err_sign = 0
	...
	do k = 2, n_table
		...
		if (new_asg_2n(p, 1) .ne. 0) then
			if (all(info_spectrum(p)%prsd_peak(:, new_asg_2n(p,1)) .ne. new_idx_2(1))) 	then
				new_asg_2n(p, 1) = 0
			end if
		end if
		if (new_asg_2n(p, 2) .ne. 0) then
			if (all(info_spectrum(p)%prsd_peak(:, new_asg_2n(p,2)) .ne. new_idx_2(2))) 	then
				new_asg_2n(p, 2) = 0
			end if
		end if
	...
	!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	!	this subroutine evaluate the new residue-to-peak connections
	!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	subroutine evaluate_peak_rsd (residue_peak, info_spectrum, connection_table, &
			& n_table, N_seq, cnn_row, table_type, ind_rsd, asg, ngood, nbad, nedge)
	!
	implicit none

	integer	n_table, N_seq, cnn_row, ind_rsd, asg, table_type, ngood, nbad, nedge
	...
	integer	tab1, tab2, col1, col2, shift_ind, shift_ind_1, shift_ind_2, ind_der, ind_tab
	integer	::	ngood_tab(n_table, 3), nbad_tab(n_table, 3), nedge_tab(n_table, 3)
	...
	do k1 = 1, cnn_row
		...
		if (tab1 .eq. table_type) then
			...
			ind_tab = tab2
			ind_der = shift_ind+2
			if (((ind_rsd+shift_ind) .gt. 0) .and. ((ind_rsd+shift_ind) .le. N_seq))then
				peak_2 = residue_peak(ind_rsd+shift_ind, tab2)
			else
				if ((shift_ind_1+ind_rsd) .gt. N_seq .or. (shift_ind_1+ind_rsd) .le. 0) then
					if ((info_spectrum(tab1)%ch_shifts(peak_1, col1) .gt. 0) .and. &
			...
			if (((ind_rsd-shift_ind) .gt. 0) .and. ((ind_rsd-shift_ind) .le. N_seq)) then
				...
				if ((ind_rsd+shift_ind_2) .gt. N_seq .or. (ind_rsd+shift_ind_2) .le. 0) then
					if ((info_spectrum(tab2)%ch_shifts(peak_2, col2) .gt. 0) .and. &
		...
		if (peak_1*peak_2 .eq. 0) then
			if ((peak_1+peak_2) .ne. 0)  nedge_tab(ind_tab, ind_der) = nedge_tab(ind_tab, ind_der) + 1	! edge assignment
		else
			delta_1 = e_1*e_1 + e_2*e_2
			delta_2 = ch_1 - ch_2
			delta_2 = delta_2*delta_2
			if (delta_1 .gt. delta_2) then
				ngood_tab(ind_tab, ind_der) = ngood_tab(ind_tab, ind_der) + 1	! good assignment
			else
				nbad_tab(ind_tab, ind_der) = nbad_tab(ind_tab, ind_der) + 1		! bad assignment
			endif
		endif
	end do
	do k1 = 1, n_table
	...
	!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	!	this subroutine calculate the pareto order of the whole group(new generation + old generation)
	!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	!	this subroutine calculate the crowding-distance of the chosen set
	!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	!
		...
		if (diff .eq. 0.0) then
			...
			do k1 = 2, size_s-1
				dist_temp(ind_array_temp(k1)) = dist_temp(ind_array_temp(k1))+(a(k1+1)-a(k1-1))/diff
			end do
		end if
	...
	!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	!	this subroutine select N individuals from N+N individuals
	!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	subroutine elitism(group_1, group_2, n_good_all, n_bad_all, n_edge_all, n_used_all, size_group, &
	& N_seq, n_table, note_rstr, group_new, pareto_order, dist_group, lucky_ratio)
	!
	use qsort_c_module
	implicit none
	!
	...
	integer, intent(in) ::	n_good_all(2*size_group), n_bad_all(2*size_group), n_edge_all(2*size_group)
	...
	integer	size_all
	integer ::	ind_temp(2*size_group), ind_temp2(2*size_group), ind_dist(2*size_group)
...
!
	...
	if (note_rstr .ge. 0) then
		...
		obj(:, 3) = obj(:,3)*lucky_ratio + (1-lucky_ratio)*(temp_one/maxval(abs(temp_one)))
!		obj(:, 4) = -n_edge_all*1.0
		num_obj = 3
	else
		obj(:, 1) = n_good_all*1.0
		obj(:, 2) = n_used_all*1.0
		...
		obj(:, 4) = obj(:,4)*lucky_ratio + (1-lucky_ratio)*(temp_one/maxval(abs(temp_one)))
!		obj(:, 5) = -n_edge_all*1.0
		num_obj = 4
	end if
	!
	! ******** pareto order ***********
	if (note_rstr .lt. 0) then
		call pareto(obj(:, 1:num_obj), num_obj, size_all, num_set, Pareto_set, rank)
	else
		call pareto_rstr(obj(:, 1:num_obj), n_bad_all, num_obj, size_all, num_set, Pareto_set, rank)
	end if
	ind_k = 0
	do k1 = 1, rank
		...
		call crowd_dist(obj(ind_temp2(1: num_set(k1)), 1:num_obj), num_obj, num_set(k1), dist_s(1:num_set(k1)))
		!
		if (size_group .ge. (num_set(k1)+ind_k)) then
			...
			dist_temp(1:num_set(k1)) = maxval(dist_s(1:num_set(k1)))-dist_s(1:num_set(k1))
	...
	!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	!	this subroutine delete the all-zero individuals and identical ones in the group
	!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	subroutine delete_z_i(residue_peak, parents, offsprings, ind_new_group, N_seq, n_table, size_group, del_note)
	!
	...
	!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	!	subroutine for evaluating each individual
	!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	implicit none
	!
	integer	n_table, N_seq, cnn_row, asg
	integer,intent(in) ::	residue_peak(N_seq, n_table)
	...
	do k1 = 1, N_seq
		do k2 = 1, n_table
			...
			call evaluate_peak_rsd(rsd_peak_temp, info_spectrum, connection_table, &
			& n_table, N_seq, cnn_row, k2, k1, asg, ngood_new, nbad_new, nedge_new)
			call evaluate_peak_rsd(rsd_peak_temp, info_spectrum, connection_table, &
			& n_table, N_seq, cnn_row, k2, k1, 0, ngood_old, nbad_old, nedge_old)
			!
