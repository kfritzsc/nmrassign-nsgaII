! Recursive Quicksort in Fortran 95
! =================================
! Sort real numbers into ascending numerical order.
!
! Author
! ------
! Juli Rew, SCD Consulting (juliana@ucar.edu), 9/03
!
! Contributor
! -----------
! Walt Brainerd, Made F conformant
!
! Reference
! ---------
! Based on algorithm from Cormen et al., Introduction to Algorithms, 1997

module qsort_c_module

implicit none
public :: QsortC
private :: Partition

contains

recursive subroutine QsortC(A, ind_A)
  real, intent(in out), dimension(:) :: A
  integer, intent(in out), dimension(:) :: ind_A
  integer :: iq

  if(size(A) > 1) then
     call Partition(A, ind_A, iq)
     call QsortC(A(:iq-1), ind_A(:iq-1))
     call QsortC(A(iq:), ind_A(iq:))
  endif
end subroutine QsortC

subroutine Partition(A, ind_A, marker)
  real, intent(in out), dimension(:) :: A
  integer, intent(in out), dimension(:) :: ind_A
  integer, intent(out) :: marker
  integer :: i, j, ind_temp
  real :: temp
  real :: x      ! pivot point
  x = A(1)
  i= 0
  j= size(A) + 1

  do
     j = j-1
     do
        if (A(j) <= x) exit
        j = j-1
     end do
     i = i+1
     do
        if (A(i) >= x) exit
        i = i+1
     end do
     if (i < j) then
        ! exchange A(i) and A(j)
        temp = A(i)
        A(i) = A(j)
        A(j) = temp
        ind_temp = ind_A(i)
        ind_A(i) = ind_A(j)
        ind_A(j) = ind_temp
     elseif (i == j) then
        marker = i+1
        return
     else
        marker = i
        return
     endif
  end do

end subroutine Partition

end module qsort_c_module
