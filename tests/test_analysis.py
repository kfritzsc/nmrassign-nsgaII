...
class AnalysisTest(unittest.TestCase):
    ...
    Use nearly ideal GB1 data to test for GA assign analysis.
    """
    def setUp(self):
        ...
        # gb1_1 data
        control_file_path = os.path.join('data', 'gb1_1', 'control_nsga.txt')
        self.control_file = os.path.join(test_data_dir, control_file_path)

        with open(self.control_file) as control_fid:
            self.params = fileio.read_control(control_fid, parse_input=True)
        ...
        with open(output_file, 'r') as output_fid:
            self.assigns = fileio.read_outdata(self.params, output_fid)

        # gb1_2 data
        control_file_path2 = os.path.join('data', 'gb1_2', 'control_nsga.txt')
        self.control_file2 = os.path.join(test_data_dir, control_file_path2)

        with open(self.control_file2) as control_fid2:
            self.params2 = fileio.read_control(control_fid2, parse_input=True)
        ...
        with open(output_file2, 'r') as output_fid2:
            self.assigns2 = fileio.read_outdata(
                self.params2, output_fid2)
    ...
    def test_pareto_filter(self):
        top_paretos = analysis.keep_pareto_order(self.scores2)
        new_assigns = analysis.pareto_filter(
            self.assigns2, top_paretos=top_paretos)

        n_top = sum([1 for x in top_paretos if x])
        self.assertEqual(len(new_assigns[0]), n_top)

    def test_assignment_stats(self):
        top_paretos = analysis.keep_pareto_order(self.scores)
        new_assigns = analysis.pareto_filter(
            self.assigns, top_paretos=top_paretos)
        mode_assignments, mode_probs = analysis.assignment_stats(
            new_assigns)
        self.assertEqual(mode_assignments[0][1], 37)
